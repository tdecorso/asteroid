...
bool engine_Init(engine_t* e) {
    ...
    if (!sdl_platform_Init(&e->platform)) return false;
    if (!audio_Init(&e->audio)) return false;
    return true;
}
